...
import {
  Share2, Download, File, FileImage, FileText, Folder, Timer, ChevronDown, ChevronRight,
} from "lucide-react";
...
const fileTypeFromName = (name: string, forceFolder = false): "folder" | "file" => {
  ...
  return /\.(pdf|pptx?|xlsx?|docx?|png|jpe?g|gif|webp|bmp|tiff|zip|7z|rar)$/.test(lower) ? "file" : "folder";
};
...
// ──────────────────────────────────────────────────────────────────────────────
// FolderTree (single definition, memoized)
// ──────────────────────────────────────────────────────────────────────────────
...
// ──────────────────────────────────────────────────────────────────────────────
// FolderContents (controlled open state)
// ──────────────────────────────────────────────────────────────────────────────
...
const SharedDocuments: React.FC = () => {
  ...
  const fetchSharedWithMe = useCallback(async () => {
    ...
    try {
      ...
      // keep sort stable folder-first
      setCards(mapped.sort((a,b) => (a.type===b.type ? a.name.localeCompare(b.name) : a.type==="folder" ? -1 : 1)));
      // NOTE: openByCard remains unchanged, so open panels stay open
    } catch (e) {
      console.error(e);
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 py-8">
        ...
        {isLoading ? (
          ...
          <div className="grid w-full grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filtered.map((card) => (
              <CardItem
                key={card.documentId} // stable key = actual document id
                card={card}
                fetchChildrenCached={fetchChildrenCached}
